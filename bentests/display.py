...
class Printer:
	...
	def tprint(self, value:Any, depth:int=0,end="",is_terminal=True): #TODO: make this take in a "type" avariable, probably type(value) <- orignal value before preparing for print, so that it can say "string" type: str.
		'''
		The print method, taking tabs into account.

		Parameters:
            value (Any): item to be printed
			depth (int): current recursion/iterable depth (in terms of iterables, classes, etc)
	...
	def _(self, variable:Enum, depth=0,end=""):
		...
		output = f"{variable.name} ({enum_type}) = {variable.value}"
